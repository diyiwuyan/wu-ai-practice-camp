...
核对实际访问范围、文件变更和产物路径；远程任务结束后关闭不必要的授权；重要动作保留操作记录。`},28:{sourceToken:"IE0PdDxKqoxWbyxfwN3cJSFonpg",markdown:`# 第 28 章 每日资讯早报：从信息采集到定时推送

## 案例目标

每天固定时间获取 AI、开源项目或竞品动态，去重、筛选、补来源后形成少量高价值通知。

## 从手动到定时

先手动运行至少三次，观察来源失效率、重复率、人工退回率和推送延迟；格式稳定后再设置定时任务。来源不稳定或 Prompt 频繁修改时，不急着自动化。

## 可直接使用的提示词

\`\`\`text
每天 9:00 汇总 AI 领域 5 条值得关注的动态。候选来源包括官方博客、GitHub、论文和可信行业媒体。先去重，再按模型、产品、开源、论文和商业化分类。每条输出标题、100 字以内摘要、原始链接、影响判断和可信度；没有可靠来源就标记待核验，不要编造。只推送最重要的 5 条，其余归档到日报文档。
\`\`\`
...
- 部分来源失败要在顶部标明缺失。
- 推送失败时保留已生成文档，不重复发送。
- 修改来源或 Prompt 后重新手动运行三次再保存自动化配置。`},29:{sourceToken:"EbMJdpon6oIFY5xNndqcRYsbnwc",markdown:`# 第 29 章 会议到交付包：纪要、任务表、PRD 和汇报
