...
文件总数守恒；冲突没有被静默覆盖；映射表可逐条追溯；重要目录先在副本上演练。`},27:{sourceToken:"PnkYdLsJRovKvWxpenLch4cunwd",markdown:`# 第 27 章 远程控制：不在电脑前也能完成交付

## 案例目标

人在外出或不在电脑前时，完成低风险的查看、整理和产物生成，同时把远程操作限制在明确的目录和动作范围内。

## 远程任务卡

- 目标电脑和在线时间。
- 允许访问的目录与禁止访问的目录。
- 允许执行的动作：读取、复制、生成、重命名或移动。
- 必须人工确认的动作：删除、覆盖、对外发送、安装软件和修改系统设置。

## 可直接使用的提示词

\`\`\`text
在远程电脑上只处理 ~/WorkBuddy/inbox，本次任务只允许读取文件、生成 output/report.md 和复制结果到 output/archive。不要访问其他目录，不要删除、覆盖、安装软件或修改系统设置。执行前先返回文件清单和计划，遇到权限、路径或内容不确定时停止并等待确认。完成后报告实际访问路径、生成文件、未完成事项和错误信息。
\`\`\`

## 验收

核对实际访问范围、文件变更和产物路径；远程任务结束后关闭不必要的授权；重要动作保留操作记录。`},28:{sourceToken:"IE0PdDxKqoxWbyxfwN3cJSFonpg",markdown:`# 第 28 章 AI 日记：白天随手记，晚上自动复盘
